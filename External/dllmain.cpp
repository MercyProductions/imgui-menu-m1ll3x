...
namespace imgui
{
	...
	ImFont* start(IDXGISwapChain3* pSwapChain, ID3D12CommandQueue* pCommandQueue, type::tImguiStyle SetStyleFunction)
	{
		...
		ImFont* main_font = io.Fonts->AddFontFromMemoryTTF(inter_semibold, sizeof(inter_semibold), 15.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
		font::inter_semibold = io.Fonts->AddFontFromMemoryTTF(inter_semibold, sizeof(inter_semibold), 15.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
		font::icomoon_page = io.Fonts->AddFontFromMemoryTTF(icomoon_page, sizeof(icomoon_page), 17.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
		font::icomoon_logo = io.Fonts->AddFontFromMemoryTTF(icomoon_page, sizeof(icomoon_page), 30.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
		font::icon_notify = io.Fonts->AddFontFromMemoryTTF(icon_notify, sizeof(icon_notify), 17.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
	...
	ImFont* add_font(const char* font_path, float font_size)
	{
		...
		ImGuiIO& io = ImGui::GetIO();
		ImFont* font = io.Fonts->AddFontFromMemoryTTF(inter_semibold, sizeof(inter_semibold), 15.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
