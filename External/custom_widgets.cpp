...
#include <string>
    ...
    std::map<ImGuiID, theme_state> themeanim;
    bool ThemeButton(const char* id_theme, bool dark, const ImVec2& size_arg)
    {
        ...
        auto it_anim = themeanim.find(id);

        if (it_anim == themeanim.end())
        {
            themeanim.insert({ id, theme_state() });
            it_anim = themeanim.find(id);
        }
    ...
    std::map<ImGuiID, button_state> Buttonanim;
    bool Button(const char* label, const ImVec2& size_arg)
    {
        ...
        {
            Buttonanim.insert({ id, button_state() });
    ...
    std::map<ImGuiID, page_state> pageanim;
    bool Page(bool selected, const char* icon, const ImVec2& size_arg)
    {
        ...
        auto it_anim = pageanim.find(id);

        if (it_anim == pageanim.end())
        {
            pageanim.insert({ id, page_state() });
            it_anim = pageanim.find(id);
        }
    ...
    std::map<ImGuiID, check_state> cgheckboxanim;
    bool Checkbox(const char* label, bool* v)
    {
        ...
        auto it_anim = cgheckboxanim.find(id);

        if (it_anim == cgheckboxanim.end())
        {
            cgheckboxanim.insert({ id, check_state() });
            it_anim = cgheckboxanim.find(id);
        }
    ...
    std::map<ImGuiID, begin_state> comboanim;
    bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        auto it_anim = comboanim.find(id);

        if (it_anim == comboanim.end())
        {
            comboanim.insert({ id, begin_state() });
            it_anim = comboanim.find(id);
        }
    ...
    std::map<ImGuiID, select_state> selectableanim;
    bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
    {
        ...
        auto it_anim = selectableanim.find(id);

        if (it_anim == selectableanim.end())
        {
            selectableanim.insert({ id, select_state() });
            it_anim = selectableanim.find(id);
        }
    ...
    std::map<ImGuiID, edit_state> editanim;
    bool ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
    {
        ...
        if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
        {
            ...
            auto it_anim = editanim.find(ImGui::GetID(label));

            if (it_anim == editanim.end())
            {
                editanim.insert({ ImGui::GetID(label), edit_state() });
                it_anim = editanim.find(ImGui::GetID(label));
            }
    ...
    std::map<ImGuiID, knob_state> knobanim;
    bool KnobScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        auto it_anim = knobanim.find(id);

        if (it_anim == knobanim.end())
        {
            knobanim.insert({ id, knob_state() });
            it_anim = knobanim.find(id);
        }
    ...
    std::map<ImGuiID, slider_state> slideranim;
    ...
    bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        {
            slideranim.insert({ id, slider_state() });
