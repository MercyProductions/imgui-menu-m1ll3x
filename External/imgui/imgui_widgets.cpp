...
#include "C:\Users\Velocity\Desktop\DEMO\ImGui_Settings.h"
...
std::map<ImGuiID, input_state> Inputanim;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    auto it_anim = Inputanim.find(id);

    if (it_anim == Inputanim.end())
    {
        Inputanim.insert({ id, input_state() });
        it_anim = Inputanim.find(id);
    }
